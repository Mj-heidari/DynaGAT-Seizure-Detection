...
from models.dynagat_model_v5 import DynaGATOnsetModelV5
...
def _hardware_summary() -> None:
    ...
    if torch.cuda.is_available():
        ...
        print("[!] CUDA not detected. Training will run on CPU and will be slow.")


def _save_predictions_v5(
...
def run_lopo_v5(
    ...
    if not cache_paths:
        raise FileNotFoundError(
            f"No v3 temporal caches found in {PROCESSED_DATA_DIR}. Run preprocessing first."
        )

    cache_by_subject: Dict[str, Dict] = {}
    print(f"[*] Loading {len(cache_paths)} v3 patient caches...")
    ...
    for fold_idx in selected_indices:
        ...
        print(
            f"TEST {fold_name}: AUROC={test_window['auroc']:.4f} | AUPRC={test_window['auprc']:.4f} | "
            f"F1={test_window['f1']:.4f}"
        )
        print(
